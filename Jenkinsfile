pipeline {
    ...
    environment {
        IMAGE = "delhivery-api:${BUILD_NUMBER}"
        NETWORK = "delhivery-net"
        MYSQL_CONT = "delhivery-mysql"
        API_CONT = "delhivery-api"
 
        MYSQL_PWD = "root"
        MYSQL_DB = "delhivery_db"
    }
        ...
        stage('Run API') {
            steps {
                ...
                docker run -d --name %API_CONT% --network %NETWORK% ^
                    ...
                    -e JWT_ISSUER=delhivery-api ^
                    -e JWT_AUDIENCE=delhivery-clone ^
                    ...
                    -e Razorpay__Key=YOUR_RAZORPAY_KEY ^
                    -e Razorpay__Secret=YOUR_RAZORPAY_SECRET ^
